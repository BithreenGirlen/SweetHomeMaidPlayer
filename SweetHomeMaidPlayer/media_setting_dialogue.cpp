...
{
	int fontSize = Constants::kFontSize * ::GetDpiForSystem() / USER_DEFAULT_SCREEN_DPI;
...
LRESULT CMediaSettingDialogue::OnSize()
{
	...
	long fontSize = Constants::kFontSize * ::GetDpiForSystem() / USER_DEFAULT_SCREEN_DPI;
	long lTextSpace = fontSize;
	...
	{
		::MoveWindow(m_hVolumeText, x_space, y_space, Constants::kTextWidth, fontSize, TRUE);
	}
	...
	{
		::MoveWindow(m_hRateText, w / 2 + x_space, y_space, Constants::kTextWidth, fontSize, TRUE);
	}
...
/*WM_COMMAND*/
LRESULT CMediaSettingDialogue::OnCommand(WPARAM wParam)
{
